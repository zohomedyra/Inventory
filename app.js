/* Inventory + Batch + Committed Dashboard (v12c CSP)
   Robust parsing, auto-detects headers, category-in-rows, expiry alerts,
   committed details for selected product (with Unused Credits).
*/

// ----------------- helpers -----------------
const $ = id => document.getElementById(id);
const fmtIN = n => { if(n==null||isNaN(n)) return "–"; const v=+n,a=Math.abs(v);
...
function num(v){ if(v==null) return NaN; if(typeof v==="number") return Number.isFinite(v)?v:NaN;
  let s=String(v).trim(); if(!s) return NaN; let neg=false;
  if(/^\(.*\)$/.test(s)){neg=true; s=s.slice(1,-1);}
  if(/-$/.test(s)){neg=true; s=s.slice(0,-1);}
  s=s.replace(/(₹|rs\.?|inr)/gi,""); s=s.replace(/,/g,"").replace(/\s+/g,"");
  s=s.replace(/\.(?=.*\.)/g,""); s=s.replace(/[^0-9.\-]/g,"");
  if(s===""||s==="."||s==="-"||s==="-.") return NaN;
  const n=Number(s); return Number.isFinite(n)?(neg?-n:n):NaN; }
function parseToISO(x){ if(x==null) return null; const n=num(x);
  if(Number.isFinite(n)){ // Excel serial / epoch
    if(n>=20000&&n<=80000){ const d=new Date(Date.UTC(1899,11,30)+n*86400000); return isNaN(d)?null:d.toISOString();}
  ...
  return null; }
function monYYYY(dlike){ const m=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
  const iso=parseToISO(dlike); if(iso){ const d=new Date(iso); return `${m[d.getUTCMonth()]}-${d.getUTCFullYear()}`; }
  const d2=new Date(dlike); if(!isNaN(d2)) return `${m[d2.getMonth()]}-${d2.getFullYear()}`; return String(dlike||""); }
function monthsLeft(expISO){ const now=new Date(); const d=new Date(expISO); if(isNaN(d)) return NaN;
  return (d.getUTCFullYear()-now.getUTCFullYear())*12 + (d.getUTCMonth()-now.getUTCMonth()); }

// ----- mismatch policy -----
const COUNT_MISSING_AS_MISMATCH = false;  // only true quantity mismatches are counted
const QTY_TOLERANCE = 1;                  // |batch - inventory| <= 1 is OK

// ----------------- state -----------------
let invRows=[], invCols=[];        // inventory
let batchRows=[], batchCols=[];    // batch
...
let commitMap={sku:"",product:"",customer:"",qty:"",unused:"",soid:"",status:""};
...
function bestCol(cols,hints){ const nc=cols.map(c=>({raw:c,n:norm(c)}));
  for(const h of hints){ const nh=norm(h);
...
function findHeaderRow(arr){ let best=0,bestCnt=0;
  for(let i=0;i<Math.min(25,arr.length);i++){ const cnt=(arr[i]||[]).filter(x=>String(x||"").trim()!=="").length;
    if(cnt>bestCnt){bestCnt=cnt;best=i;} } return best; }
function looksLikeLabel(text){ const s=String(text||"").trim(); if(!s) return false; if(/\d/.test(s)) return false;
  if(s.length>2&&s===s.toUpperCase()) return true; return /^[A-Za-z][A-Za-z\s/&\-()]+$/.test(s); }
...
function buildInvFromArrays(arr){
  ...
  let prodIdx=headers.findIndex(h=>/item|product|name|description/i.test(h)); if(prodIdx<0) prodIdx=0;
  ...
  for(let i=start;i<clean.length;i++){
    ...
    if((!skuCell||String(skuCell).trim()==="") && looksLikeLabel(prodCell)){ currentCategory=String(prodCell||"").trim(); continue; }
    const o={Category:currentCategory}; headers.forEach((h,idx)=>{ if(h) o[h]=r[idx]; });
...
function buildBatchFromArrays(arr){
  ...
  for(let i=start;i<clean.length;i++){ const r=clean[i]||[]; const o={}; headers.forEach((h,idx)=>{ if(h) o[h]=r[idx]; });
...
function initSettings(){
  ...
  rebuildSCProducts();
  if($("detailCat")) $("detailCat").innerHTML = `<option value="">All</option>` + cats.map(c=>`<option>${c}</option>`).join("");
}
function rebuildSCProducts(){
  const scCategory=$("scCategory"); const scProduct=$("scProduct"); const scSub=$("scProductSub");
  if(!scCategory||!scProduct) return;
  const catSel=scCategory.value;
  const pcol=invMap.product||invMap.salesDesc, skuCol=invMap.sku;
  const items=invRows.filter(r=>String(r.Category||"(Uncategorized)")===(catSel||""))
...
// ----------------- filters (used for mismatch/expiry) -----------------
function filteredSKUSet(){
  const set=new Set();
  const skuCol=invMap.sku;
  for(const r of invRows){ const s=normalizeSKU(r[skuCol]); if(s) set.add(s); }
  return set;
}

// ----------------- merge & compute (mismatch policy applied) -----------------
...
function mergeAndCompute(){
  ...
  for(const b of batchRows){
    ...
    const obj={_skuRaw:skuRaw,_sku:sku,_qty:num(b[qB]),_batch:String(b[bNo]||"").trim(),_mfgRaw:b[mfg],_mfgISO:parseToISO(b[mfg]),_expRaw:b[exp],_expISO:parseToISO(b[exp])};
  ...
  for(const sku of allSKUs){
    ...
    if(hasInv && hasBatch){
      ...
      let reason="";
      if(hasInv && !hasBatch) reason="Missing in batch file";
      if(!hasInv && hasBatch) reason="Missing in inventory";
      if(reason){
        mismatchRows.push({
...
function mismatchTable(){
  ...
  el.innerHTML=`<table><thead><tr><th>SKU</th><th>Product</th><th>Category</th><th>Stock On Hand</th><th>Batch Available</th><th>Diff</th><th>Reason</th></tr></thead><tbody>
...
function expiryTable(){
  ...
  for(const b of batchRows){
    ...
    rows.push({SKU:inv? (inv[invMap.sku]) : sku, Product:prod, Batch:String(b[bNo]||""), Mfg:monYYYY(b[mfg]), Exp:monYYYY(expISO), Avail:Number(num(b[qty])||0), Months:mLeft});
  }
  if(!rows.length){ el.innerHTML='<div class="muted">No products expiring in < 11 months.</div>'; return; }
  rows.sort((a,b)=>a.Months-b.Months);
  el.innerHTML=`<table><thead><tr><th></th><th>SKU</th><th>Product</th><th>Batch No</th><th>Mfg</th><th>Expiry</th><th>Available Qty</th><th>Months left</th></tr></thead>
    <tbody>${rows.map(r=>`<tr>
...
function detailsTable(){
  ...
  el.innerHTML=`<table><thead><tr>${c.map(x=>`<th>${x}</th>`).join("")}</tr></thead>
...
function renderCommittedForSelected(invRow){
  ...
  const skuCol = committedCols.find(c=>/sku/i.test(c)) || "sku";
  const prodCol= committedCols.find(c=>/item name|product name|product/i.test(c)) || "Item Name";
  const custCol= committedCols.find(c=>/customer/i.test(c)) || "Customer Name";
  const qtyCol = committedCols.find(c=>/committed/i.test(c)) || "Committed Stock";
  const unusedCol= committedCols.find(c=>/unused.*credit/i.test(c)) || "unused_credits_receivable_amount_bcy";
  ...
  }).map(r=>({Customer:String(r[custCol]||""), Qty:num(r[qtyCol]), Unused:num(r[unusedCol])}));

  if(!rows.length){ el.innerHTML='<div class="muted">No committed rows found for this product.</div>'; return; }

  const agg = new Map();
  for(const r of rows){ const key=r.Customer||"(Unknown)"; const cur=agg.get(key)||{Customer:key,Qty:0,Unused:0}; cur.Qty += Number.isFinite(r.Qty)?r.Qty:0; cur.Unused += Number.isFinite(r.Unused)?r.Unused:0; agg.set(key,cur); }
  ...
  el.innerHTML = `
    <div class="muted" style="margin-bottom:6px">Committed Stock Details (for selected product)</div>
    <div class="box" style="max-height:320px">
      <table>
        <thead><tr><th>Customer Name</th><th>Committed Qty</th><th>Unused Credits</th></tr></thead>
        <tbody>${out.map(r=>`<tr><td>${r.Customer}</td><td>${Number.isFinite(r.Qty)?r.Qty.toLocaleString('en-IN'):""}</td><td>${Number.isFinite(r.Unused)?fmtIN(r.Unused):""}</td></tr>`).join("")}</tbody>
...
function renderBatchesForSelected(invRow){
  ...
  const sku=normalizeSKU(invRow[invMap.sku]); if(!sku){ el.innerHTML=""; return; }

  const skuCol=batchMap.sku, qCol=batchMap.qty, bNo=batchMap.batchNo, mfg=batchMap.mfg, exp=batchMap.exp;
  const rows=batchRows.filter(r=>normalizeSKU(r[skuCol])===sku).map(r=>{
    const q=num(r[qCol]); return {Batch:String(r[bNo]||""), Mfg:monYYYY(r[mfg]), Exp:monYYYY(parseToISO(r[exp])||r[exp]), Qty:Number.isFinite(q)?q:0};
  });
  if(!rows.length){ el.innerHTML='<div class="muted">No batch details found for this SKU.</div>'; return; }
  const total=rows.reduce((a,b)=>a+(b.Qty||0),0);
  el.innerHTML=`
    <div class="muted" style="margin-bottom:6px">Batches for <b>${invRow[invMap.sku]||sku}</b> (${rows.length}) • Total available qty: <b>${total.toLocaleString('en-IN')}</b></div>
    <div class="box" style="max-height:320px">
      <table>
        <thead><tr><th>#</th><th>Batch No</th><th>Mfg</th><th>Expiry</th><th>Available Qty</th><th>% of SKU</th></tr></thead>
        <tbody>${rows.map((r,i)=>`<tr><td>${i+1}</td><td>${r.Batch}</td><td>${r.Mfg}</td><td>${r.Exp}</td><td>${r.Qty.toLocaleString('en-IN')}</td><td>${total?((r.Qty/total)*100).toFixed(1):"0.0"}%</td></tr>`).join("")}</tbody>
      </table>
    </div>`;
}

function updateStockCheck(){
  const scProduct=$("scProduct"); if(!scProduct || !scProduct.value){ const cards=$("scCards"); if(cards) cards.style.display="none"; $("lookupCommitted").innerHTML=""; $("lookupBatches").innerHTML=""; return; }
  const prod=scProduct.value;
  const inv=invRows.find(r=>String(r[invMap.product]||r[invMap.salesDesc]||"").trim()===prod);
  if(!inv){ const cards=$("scCards"); if(cards) cards.style.display="none"; $("lookupCommitted").innerHTML=""; $("lookupBatches").innerHTML=""; return; }
...
function refresh(){
  mergeAndCompute();
  kpis(); topProductsChart(); mismatchTable(); expiryRender(); detailsRender();
  initSettings();
...
function autoDetectAll(){ autoDetectInventory(); autoDetectBatch(); autoDetectCommitted(); initSettings(); refresh(); }
